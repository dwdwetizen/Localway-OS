...
function callbackPage(origin: string, success: boolean, message: string) {
  ...
  const html = `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><title>Google Ads</title>
  ...
  <body><div class="card"><h1>${success ? 'Google Ads conectado' : 'Não foi possível conectar'}</h1><p>${safeMessage}</p><p>Esta janela pode ser fechada.</p></div>
  <script>window.opener?.postMessage({type:'localway-google-ads',success:${success}},${JSON.stringify(origin)});setTimeout(()=>window.close(),1800);</script></body></html>`;
...
export async function GET(request: NextRequest) {
  ...
  if (!state) return callbackPage(fallbackOrigin, false, 'A autorização expirou. Inicie a conexão novamente.');
  ...
  return callbackPage(state.origin, true, `A conta ${connectedEmail || 'Google Ads'} foi autorizada com sucesso.`);
}
