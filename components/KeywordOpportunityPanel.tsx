      ...
      <div className="rounded-2xl bg-[#f8f9fc] dark:bg-[#10142e] p-4 border border-[#c2c6d8]/30">
        ...
        {!idea ? <div className="py-12 text-center text-xs text-[#727687]"><BarChart3 className="w-8 h-8 mx-auto mb-2 opacity-50"/>Consulte e selecione uma palavra-chave para calcular.</div> : <>
          ...
          <div className="grid grid-cols-2 gap-3 mt-3">
            <NumberField label={`CTR atual • posição ${effectivePosition ? effectivePosition.toFixed(1) : '20+/não medida'}`} help="Estimativa de quantas pessoas clicam ao ver a empresa na posição atual." value={currentCtr} disabled suffix="%"/>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 mt-3 text-center">
            <Metric label="Faturamento estimado atual" value={currency(currentRevenue)} help="Estimativa mensal com a posição e as taxas informadas acima."/>
            <Metric label="Potencial estimado Top 3" value={currency(topRevenue)} help="Estimativa mensal se o CTR alcançar a meta de Top 3."/>
            <Metric label="Oportunidade estimada" value={currency(lostRevenue)} help="Diferença entre os dois cenários. Não é perda comprovada." accent/>
          </div>
...
function NumberField({ label, help, value, onChange, prefix, suffix, disabled = false }: {
  label: string;
  help: string;
  value: number;
