...
export function AdminView({ onShowToast }: AdminViewProps) {
  ...
  return <div className="space-y-6 animate-in fade-in duration-300">
    ...
    {activeTab === 'integracoes' && <section className="bg-white dark:bg-[#141936] p-6 rounded-2xl border"><Settings className="text-[#0066ff]"/><h3 className="font-bold mt-3">Integrações</h3><p className="text-xs text-[#727687] mt-1">Cole as chaves uma vez. Elas ficarão salvas para todos os navegadores da equipe.</p><div className="mt-4 grid gap-3 md:grid-cols-3"><IntegrationStatus configured={placesConfigured} title="Google Places API (New)" ready="Geração de leads e análise de perfis prontas." missing="Cole abaixo a chave da Places API (New)."/><IntegrationStatus configured={mapsConfigured} title="Google Maps JavaScript API" ready="Mapa e grade de visibilidade prontos." missing="Cole abaixo a chave do Maps JavaScript."/><IntegrationStatus configured title="Google Agenda" ready="Ativo por link seguro, sem chave de API." missing=""/></div><div className="mt-5 grid gap-4 md:grid-cols-2"><label className="text-xs font-bold">Chave secreta — Places API<input type="password" autoComplete="off" value={placesKeyInput} onChange={event => setPlacesKeyInput(event.target.value)} placeholder={placesConfigured ? 'Configurada — cole somente para substituir' : 'Cole a chave do Google Places'} className="input mt-1"/><span className="block text-[10px] font-normal text-[#727687] mt-1">Usada apenas pelo servidor; colaboradores não conseguem visualizá-la.</span></label><label className="text-xs font-bold">Chave do mapa — Maps JavaScript API<input type="password" autoComplete="off" value={mapsKeyInput} onChange={event => setMapsKeyInput(event.target.value)} placeholder={mapsConfigured ? 'Configurada — cole somente para substituir' : 'Cole a chave do Google Maps'} className="input mt-1"/><span className="block text-[10px] font-normal text-[#727687] mt-1">Restrinja esta chave aos domínios do aplicativo no Google Cloud.</span></label></div><button disabled={savingIntegrations || (!placesKeyInput.trim() && !mapsKeyInput.trim())} onClick={() => void saveIntegrations()} className="mt-5 px-5 py-2.5 bg-[#0066ff] disabled:opacity-50 text-white rounded-xl text-xs font-bold flex items-center gap-2">{savingIntegrations ? <Loader2 className="w-4 h-4 animate-spin"/> : <Settings className="w-4 h-4"/>}Salvar integrações</button><IntegrationGuide/></section>}
  </div>;
...
function IntegrationGuide() {
  return <div className="mt-6 rounded-2xl border border-[#c2c6d8]/40 bg-[#f4f2fd]/60 dark:bg-[#10142e] p-5">
    ...
    <ol className="mt-3 space-y-3 text-[11px] text-[#424656] dark:text-[#b0b4ce] list-decimal pl-4">
      ...
      <li><strong>Places:</strong> crie uma chave, restrinja-a à Places API (New), cole no primeiro campo e salve. Ela gera leads e consulta os dados usados nas análises.</li>
      <li><strong>Mapa:</strong> crie uma segunda chave, restrinja à Maps JavaScript API e aos endereços do seu aplicativo na Vercel, cole no segundo campo e salve.</li>
